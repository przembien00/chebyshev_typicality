...
bool store_list( const hid_t ID, const std::string& name, const T& value_list )
{
    ...
    auto dataset_id = H5Dcreate2( ID, name.c_str(), H5type, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
    auto status = H5Dwrite(dataset_id, H5type, H5S_ALL, H5S_ALL, H5P_DEFAULT, value_list.data());
    H5Dclose( dataset_id );
    H5Sclose( space_id );
    return (status < 0) ? false : true;
}
