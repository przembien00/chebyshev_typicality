...
markers = ['v', '^', 's', 'x', 'D', 'p']
fig, (ax_xx, ax_zz) = plt.subplots(nrows=2, ncols=1, sharex=True, figsize=(8, 6), gridspec_kw={'hspace': 0})
...
for beta in beta_array:
    ...
    all_sdmft, times_sdmft = ImportData_spinDMFT("ISO", physical_data=f"JL=-2__beta={beta:.2g}__h=z_h_abs={h_z:.1g}", project="spinDMFT", extension="")
    Re_G = np.array( [ gab for gab in all['results']['Re_correlation']] )
    Im_G = np.array( [ gab for gab in all['results']['Im_correlation']] )
    Re_G_err = np.array( [ gab for gab in all['results']['Re_stds']] )
    Im_G_err = np.array( [ gab for gab in all['results']['Im_stds']] )
    ...
    ax_xx.errorbar(times, G_xx, yerr=G_xx_err, ls=':', label = rf'Chebyshev, $\beta J_Q$={beta:.2g}', marker=markers[i], markevery=25, color='blue', errorevery=5)
    ...
    ax_zz.errorbar(times, G_zz, yerr=G_zz_err, ls=':', label = rf'Chebyshev, $\beta J_Q$={beta:.2g}', marker=markers[i], markevery=25, color='blue', errorevery=5)
    i+=1
...
label_list = []
for beta in beta_array:
    label_list.append(rf'$k_BT/J_Q$={1/beta:.2g}')

for ax in [ax_xx, ax_zz]:
...
fig.savefig(foldername+"stusti_stack.pdf", dpi=1000)