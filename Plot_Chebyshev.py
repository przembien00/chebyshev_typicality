...
cutoff_array = [5, 7, 10, 12]

all_conv, times = ImportData(f"ISO__Square_NN_PBC_N=16__beta=5__rescale=0.5__Cheb_cut=12")
G_conv = np.array( [ gab for gab in all_conv['results']['g_zz']] )


for n in cutoff_array:

    all_1, times = ImportData(f"ISO__Square_NN_PBC_N=16__beta=5__rescale=0.5__Cheb_cut={n}")
    ...
    plt.plot(times, np.abs(G_1), label = rf'Chebyshev cutoff={n}')
